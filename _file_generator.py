...
#                is_in_dict = 1
            ...
            elif meas_floor in floor_slaves.keys():		# Если такое название уже есть , то будет номер.1->.2->.3
                '''Второй и более вход для всех'''
...
def get_one_floor_data(meas_list):
	...
	for meas in meas_list:
		...
		if slave_number == 1:
			...
			ul_3g = get_ul(meas) 			# ok методика средней скорости?)
...
''' Получение среднего значения MOS'''
def get_mos_dl(meas):
